...
//--------------------------------------------------------------------------------
 ...
 //Affichage dégradé de niveaux de gris dans Graph2D
  ...
  int func = 3; // Decides on which approximation to use
...
double fcmv(double cmv, double* y, int N) {
  ...
  return sum1 / sum2 - 1 / cmv - 1.0/N * sum3;
}
...
double dfcmv(double cmv, double* y, int N) {
  ...
  return (sum1*sum3 - CARRE(sum2)) / CARRE(sum3) + 1/CARRE(cmv);
}
  ...
  for (int k=0; k < width; k++) {
    for (int l=0; l < length; l++) {
      ...
      for (int i=0; i < NbIterMax; i++) {
        ...
        z_n = sqrt(CARRE(z_xk) + CARRE(z_yl));
