...
float resultat_recursion(float* vecteur, int size, float PI){
  float somme = somme_recursive(vecteur, size);
...
float approx_pi(float x_0, float PI){
  float sum = 0;
  int n = 10000000;
  float u = 4.0;
  float x_n = x_0;
...
// float* v = fmatrix_allocate_1d(NBINTERV+1);
...
// bifurcation(Graph2D, width, length, 20000);
 float sum1 = approx_pi(0.2, PI);
 float sum2 = approx_pi(0.4, PI);
 float sum3 = approx_pi(0.6, PI);
