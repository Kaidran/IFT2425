...
// VxM voisins et VyM voisins
void neighborAvgMovement(float** VxM, float** VyM,
                         float** vx, float** vy,
                         int len, int width)
{
  int i,j;
  for (i=1; i < len-1; i++)
    for (j=1; j < width-1; j++)
  {
    VxM[i][j]=( vx[i-1][j] + vx[i+1][j]   + vx[i][j+1]   + vx[i][j-1])/6.0 +
            ( vx[i-1][j-1] + vx[i-1][j+1] + vx[i+1][j+1] + vx[i+1][j-1])/12.0;

    VyM[i][j]=( vy[i-1][j] + vy[i+1][j]   + vy[i][j+1]   + vy[i][j-1])/6.0 +
            ( vy[i-1][j-1] + vy[i-1][j+1] + vy[i+1][j+1] + vy[i+1][j-1])/12.0;
  }
}

// Ix Iy It differences
void compareTwoImgs(float** img1, float** img2,
                    ...
                    int len, int width)
{
    ...
    for (j=0; j < width-1; j++)
  {
    Ix[i][j] =( img1[i][j+1] - img1[i][j]
             + img1[i+1][j+1] - img1[i+1][j]
             + img2[i][j+1] - img2[i][j]
             + img2[i+1][j+1] - img2[i+1][j])/4.0;

    Iy[i][j] =( img1[i+1][j] - img1[i][j]
              + img1[i+1][j+1] - img1[i][j+1]
              + img2[i+1][j] - img2[i][j]
              + img2[i+1][j+1] - img2[i][j+1])/4.0;

    It[i][j] =( img2[i][j] - img1[i][j]
              + img2[i+1][j] - img1[i+1][j]
              + img2[i][j+1] - img1[i][j+1]
              + img2[i+1][j+1] - img1[i+1][j+1])/4.0;
  }
}
// jacobi iteration

void jacobi(float** OptFl_Vx, float** OptFl_Vy,
            float** VxM, float** VyM,
            float** Ix, float** Iy, float** It,
            int len, int width, float alpha)
{
  int i,j;
  float value;
  for (i=0; i < len; i++)
    for (j=0; j < width; j++)
    {
      value = (Ix[i][j] * VxM[i][j] + Iy[i][j] * VyM[i][j] + It[i][j])
            / (alpha*alpha + Ix[i][j] * Ix[i][j] + Iy[i][j] * Iy[i][j]);

      OptFl_Vx[i][j] = VxM[i][j] - Ix[i][j] * value;
      OptFl_Vy[i][j] = VyM[i][j] - Iy[i][j] * value;
    }
}
//----------------------------------------------------------
//----------------------------------------------------------
...
/*    for(k=0;k<NBITER;k++) 
 ...
 //Programmer ici ........
  ...
  compareTwoImgs(Img1,Img2,Ix,Iy,It,length,width);

  for (k=1; k < NBITER; k++)
  {
      neighborAvgMovement(VxM,VyM,OptFl_Vx[k-1],OptFl_Vy[k-1],length,width);

//      printf("%f \n",VxM[0][0]);
      jacobi(OptFl_Vx[k], OptFl_Vy[k],VxM, VyM, Ix,Iy,It,length,width,alpha);
  }
